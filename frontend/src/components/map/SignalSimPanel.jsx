...
function TrafficLight({ active }) {
  return (
    <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
      ...
      <div style={{
        width: 22, height: 22, borderRadius: "50%",
        background: !active ? "#ef4444" : "var(--bg2)",
...
export default function SignalSimPanel({ intNo, intNm, onPhaseChange, phaseOverride, onContextChange }) {
  ...
  if (loading) return (
    <div style={{ padding: 20, color: "var(--ink2)", fontSize: 13, textAlign: "center" }}>
      신호 데이터 로딩 중...
    </div>
  );

  if (!ctx || ctx.error) return (
    <div style={{ padding: 20, color: "var(--ink3)", fontSize: 13, textAlign: "center" }}>
      {ctx?.error || "신호 데이터 없음"}
    </div>
  );
  ...
  return (
    <div style={{ fontSize: 13, color: "var(--ink0)", height: "100%", overflowY: "auto" }}>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: "var(--blu)" }}> {intNm}</div>
        <div style={{ fontSize: 12, color: "var(--ink2)", fontFamily: "monospace" }}>{now.toLocaleTimeString("ko-KR")}</div>
      </div>
        ...
        {displayPhases.map(p => {
          ...
          return (
            <div key={p.no} style={{
              ...
              background: isActive ? "rgba(34,197,94,0.08)" : "var(--bg1)",
              border: `1px solid ${isActive ? "rgba(34,197,94,0.4)" : "var(--line)"}`,
              transition: "all 0.3s"
            }}>
              ...
              <span style={{ fontSize: 11, color: "var(--ink3)", fontFamily: "monospace" }}>{p.sec}s</span>
            </div>
          );
        })}
      </div>

      <div style={{ fontSize: 11, color: "var(--ink3)", borderTop: "1px solid var(--line)", paddingTop: 8 }}>
