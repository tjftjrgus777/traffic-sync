...
function speedColor(speed) {
  if (!Number.isFinite(speed)) return "var(--ink3)";
...
function DirCard({ dir, label, arrow, signals, speeds, elapsed }) {
  ...
  return (
    <div style={{
      ...
      {/* 방향 라벨: 화살표 + 방향명 */}
      <div style={{ fontSize: 15, fontWeight: 800, color: "var(--blu)", lineHeight: 1.1 }}>
        {arrow} {label}
      </div>
        ...
        {activeSigs.map(({ key, label: sLabel }) => {
          ...
          return (
            <div key={key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minWidth: 0 }}>
              ...
              />
              {/* 신호 종류 라벨 (직진 / 좌회전 / 보행) */}
              <div style={{ fontSize: 13, color: "var(--ink2)", whiteSpace: "nowrap" }}>{sLabel}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{
        ...
        borderTop: "1px solid var(--line)", paddingTop: 6, marginTop: 2,
      }}>
        {hasSpeed ? (
          <>
            ...
            <span style={{ fontSize: 11, color: "var(--ink2)" }}>km/h 진입</span>
          </>
        ) : (
          <span style={{ fontSize: 11, color: "var(--ink3)" }}>속도 수집 중</span>
        )}
      </div>
    </div>
  );
...
export default function SignalPanel({ cr }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%", overflow: "hidden" }}>
      ...
      {/* API 수집 시각 표시 */}
      <div style={{ fontSize: 13, color: "var(--ink3)" }}>API 수집: {ts}</div>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center", minWidth: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, color: "var(--blu)", fontWeight: 700, textAlign: "center", wordBreak: "keep-all", lineHeight: 1.3 }}>
            {cr.crsrdNm}
          </div>
          ...
          <div style={{
            ...
            background: `conic-gradient(${riskGradeColor} ${riskPct}%, var(--line) 0)`,
