...
export default function VehicleSignalPanel({
  ...
  if (loading) return <div style={{ padding: "12px 0", fontSize: 12, color: "var(--ink3)", textAlign: "center" }}>신호 데이터 로딩 중...</div>;
  if (!phases.length) return <div style={{ padding: "22px 10px", color: "var(--ink3)", fontSize: 13, lineHeight: 1.7, textAlign: "center" }}>신호계획 데이터가 비어 있습니다.</div>;
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <div style={{ color: "var(--blu)", fontWeight: 900, fontSize: 15 }}>{intNm}</div>
        <div style={{ color: "var(--ink2)", fontSize: 11 }}>{now.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}</div>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", color: "var(--ink3)", fontSize: 11, marginBottom: 4 }}>
        <span>사이클 진행</span>
        <span>{Math.floor(elapsed)}s / {cycleVal}s {remainSec ? `(잔여 ${remainSec}s)` : ""}</span>
      </div>

      {/* 사이클 진행 바 */}
      <div style={{ height: 4, background: "var(--line)", borderRadius: 999, overflow: "hidden", marginBottom: 9 }}>
      ...
      {activeVehicleSignal && vehiclePhaseNo != null && (
        <div style={{
          ...
          color: "var(--ink1)", fontSize: 11,
        }}>
          <span>🚗 차량 추종 예정: <b style={{ color: "var(--ink0)" }}>현시 {vehiclePhaseNo}</b></span>
        ...
        {phases.map(phase => {
          ...
          return (
            <div key={phase.no} style={{
              ...
              {/* 초록/빨간 신호등 인디케이터 */}
              <span style={{ width: 18, height: 18, borderRadius: "50%", background: isCurrent ? "#22c55e" : "#1e293b", border: `1px solid ${isCurrent ? "rgba(34,197,94,0.65)" : "rgba(148,163,184,0.28)"}`, boxShadow: isCurrent ? "0 0 10px rgba(34,197,94,0.55)" : "none", flexShrink: 0 }} />
              <span style={{ width: 18, height: 18, borderRadius: "50%", background: isCurrent ? "#1e293b" : "#ef4444", border: `1px solid ${isCurrent ? "rgba(148,163,184,0.28)" : "rgba(239,68,68,0.55)"}`, boxShadow: !isCurrent ? "0 0 10px rgba(239,68,68,0.45)" : "none", flexShrink: 0 }} />

              <span style={{ color: isVehicleFollowing ? "var(--ink0)" : isCurrent ? "#22c55e" : "var(--ink2)", fontSize: 12, fontWeight: isVehicleFollowing || isCurrent ? 900 : 700, minWidth: 42 }}>
                현시 {phase.no}
              </span>

              {(isCurrent || isVehicleFollowing) && (
                <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                  {isCurrent && <span style={{ fontSize: 9, padding: "2px 5px", borderRadius: 999, background: "rgba(34,197,94,0.16)", border: "1px solid rgba(34,197,94,0.32)", color: "#86efac", fontWeight: 800 }}>현재</span>}
                  {isVehicleFollowing && (
                    <span style={{ fontSize: 9, padding: "2px 5px", borderRadius: 999, background: isVehicleStopPhase ? "rgba(239,68,68,0.15)" : "rgba(59,130,246,0.16)", border: `1px solid ${isVehicleStopPhase ? "rgba(239,68,68,0.32)" : "rgba(59,130,246,0.32)"}`, color: isVehicleStopPhase ? "#fca5a5" : "#93c5fd", fontWeight: 800 }}>추종</span>
                  )}
                </div>
              )}
              ...
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", minWidth: 0, flex: 1 }}>
                {(phase.dirs || []).map((dir, idx) => (
                  <span key={`${phase.no}-${idx}`} style={{ fontSize: 10, padding: "2px 6px", borderRadius: 3, background: isVehicleFollowing ? "rgba(34,197,94,0.14)" : "rgba(78,166,255,0.1)", border: `1px solid ${isVehicleFollowing ? "rgba(34,197,94,0.25)" : "rgba(78,166,255,0.2)"}`, color: isVehicleFollowing ? "#86efac" : "#4ea6ff", whiteSpace: "nowrap" }}>
                    {dir}
                  </span>
                ))}
              </div>

              <span style={{ marginLeft: "auto", color: "var(--ink3)", fontSize: 11, fontFamily: "monospace" }}>{phase.sec}s</span>
            </div>
          );
        })}
      </div>
