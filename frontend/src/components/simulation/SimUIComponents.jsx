...
export function tabButtonStyle(active, enabled) {
  return {
    ...
    border: `1px solid ${active ? "rgba(96,165,250,0.65)" : "var(--line)"}`,
    background: active ? "rgba(96,165,250,0.18)" : "var(--bg1)",
    color: !enabled ? "var(--ink3)" : active ? "#93c5fd" : "var(--ink2)",
...
export function slideButtonStyle(enabled) {
  return {
    ...
    border: `1px solid ${enabled ? "rgba(245,158,11,0.55)" : "var(--line)"}`,
    background: enabled ? "rgba(245,158,11,0.12)" : "var(--bg1)",
    color: enabled ? "#fbbf24" : "var(--ink3)",
    cursor: enabled ? "pointer" : "default",
...
export function RoutePointCard({ type, title, crossroad, empty }) {
  ...
  return (
    <div style={{ padding: 10, borderRadius: 5, background: "var(--bg1)", border: `1px solid ${crossroad ? color + "66" : "var(--line)"}` }}>
      <div style={smallLabel}>{emoji} {title}</div>
      <div style={{ fontSize: 14, fontWeight: 800, color: crossroad ? color : "var(--ink3)" }}>{crossroad?.intNm || empty}</div>
      {crossroad && <div style={{ marginTop: 4, fontSize: 11, color: "var(--ink3)", fontFamily: "monospace" }}>INT_NO: {crossroad.intNo}</div>}
    </div>
  );
}
...
// 메트릭 표시 박스 (거리, 시간 등)
export function MetricBox({ label, value, color = "var(--ink0)", sub, animate = false }) {
  ...
  return (
    <div style={{ padding: 10, borderRadius: 5, background: "var(--bg1)", border: "1px solid var(--line)" }}>
      <div style={smallLabel}>{label}</div>
      <div style={{ fontSize: 18, fontWeight: 900, color, transition: "color 0.4s" }}>{displayValue}</div>
      {sub && <div style={{ marginTop: 2, fontSize: 11, color: "var(--ink3)" }}>{sub}</div>}
    </div>
  );
}
    ...
    <div style={{ ...cardStyle, padding: 12 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, textAlign: "center" }}>
          ...
          <div style={{ marginTop: 4, color: "var(--ink0)", fontSize: 14, fontWeight: 800, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {current?.intNm || "선택 필요"}
          </div>
          {current?.intNo && <div style={{ marginTop: 3, color: "var(--ink3)", fontSize: 10, fontFamily: "monospace" }}>INT_NO: {current.intNo}</div>}
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 5, marginTop: 10 }}>
        {waypoints.map((_, idx) => (
          <button key={idx} onClick={() => onChange(idx)} style={{ width: idx === currentIndex ? 18 : 7, height: 7, borderRadius: 999, border: "none", background: idx === currentIndex ? "#f59e0b" : "var(--line2)", cursor: "pointer", padding: 0, transition: "all 0.2s" }} />
        ))}
      </div>
    </div>
  );
...
export function AnalysisLoadingBlock() {
  return (
    <span style={{ color: "var(--ink2)", display: "flex", alignItems: "center", gap: 6 }}>
      {[0, 1, 2].map(i => (
