...
export default function SimSliderPanel({
  ...
  if (loading) return <div style={{ padding: "12px 0", fontSize: 12, color: "var(--syncro-ink2)", textAlign: "center" }}>슬라이더 데이터 로딩 중...</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: autoAnimating ? "#22c55e" : "var(--syncro-ink2)" }}>신호 시뮬레이션 조정</div>
      ...
      {phases.map(p => {
        ...
        return (
          <div key={p.no} style={{ background: "var(--syncro-bg2)", border: `1px solid ${changed ? "rgba(245,158,11,0.4)" : "var(--syncro-line)"}`, borderRadius: 5, padding: "8px 10px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: "var(--syncro-ink2)" }}>현시 {p.no}</span>
                {p.dirs?.map((d, i) => (
              ...
              <span style={{ fontSize: 11, fontFamily: "monospace", color: changed ? "#f59e0b" : "var(--syncro-ink2)" }}>
