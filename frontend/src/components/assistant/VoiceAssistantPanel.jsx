...
export default function VoiceAssistantPanel({
  ...
  return (
    <div style={{
      ...
      {/* ── 헤더 ── */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: '1px solid var(--line)', flexShrink: 0 }}>
        <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--bg2)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>✦</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink0)' }}>AI 교통 어시스턴트</div>
          <div style={{ fontSize: 11, color: 'var(--ink3)', marginTop: 1 }}>{VOICE_STATUS_LABEL[voiceUI.status]}</div>
        </div>

        {/* 마이크 토글 (AI 처리 중엔 비활성) */}
        <button onClick={onStartSTT} title={voiceSTTActive ? '마이크 중단' : '마이크로 말하기'} disabled={micBlocked}
          style={{
            background: voiceSTTActive ? 'rgba(255,60,60,0.2)' : micBlocked ? 'var(--bg0)' : 'var(--bg1)',
            border: voiceSTTActive ? '1px solid rgba(255,60,60,0.5)' : '1px solid var(--line)',
            borderRadius: 6, padding: '4px 9px',
            cursor: micBlocked ? 'not-allowed' : 'pointer',
            color: voiceSTTActive ? 'rgba(255,100,100,0.9)' : micBlocked ? 'var(--ink3)' : 'var(--ink2)',
        ...
        {/* 최소화 / 닫기 */}
        <button onClick={onMinimize} style={{ background: 'none', border: 'none', color: 'var(--ink3)', fontSize: 16, cursor: 'pointer', padding: '0 2px', lineHeight: 1 }}>−</button>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--ink3)', fontSize: 16, cursor: 'pointer', padding: '0 2px', lineHeight: 1 }}>✕</button>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {voiceUI.messages.map((m, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
            <div style={{
              ...
              color: 'var(--ink0)', whiteSpace: 'pre-line', wordBreak: 'break-word',
              borderRadius: m.role === 'user' ? '12px 12px 3px 12px' : '3px 12px 12px 12px',
              background: m.role === 'user' ? 'var(--bg2)' : 'var(--bg1)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '4px 2px' }}>
            {voiceUI.steps.map((s, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--ink3)', lineHeight: 1.5, wordBreak: 'break-all' }}>{(s.label || s.content || '').slice(0, 80)}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {showProgress && (
          <div style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '6px 0' }}>
            {[0, 1, 2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--ink3)', animation: `brDot 1.2s ease ${i * .2}s infinite`, display: 'inline-block' }} />)}
            <span style={{ fontSize: 12, color: 'var(--ink3)', marginLeft: 6 }}>{voiceUI.status === 'thinking' ? '분석 중...' : '읽는 중...'}</span>
          </div>
        )}
      ...
      {voiceUI.status === 'email_confirm' && (
        <div style={{ padding: '10px 16px 12px', borderTop: '1px solid var(--line)', flexShrink: 0 }}>
          <div style={{ fontSize: 11, color: 'var(--ink3)', textAlign: 'center', marginBottom: 8 }}>버튼으로 선택해주세요</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => onEmailConfirm(true)} style={{ flex: 1, padding: '9px 0', borderRadius: 8, fontSize: 13, fontWeight: 600, background: 'var(--bg2)', border: '1px solid var(--line2)', color: 'var(--ink0)', cursor: 'pointer', fontFamily: 'inherit' }}>네</button>
            <button onClick={() => onEmailConfirm(false)} style={{ flex: 1, padding: '9px 0', borderRadius: 8, fontSize: 13, background: 'transparent', border: '1px solid var(--line)', color: 'var(--ink3)', cursor: 'pointer', fontFamily: 'inherit' }}>아니요</button>
          </div>
        </div>
      )}
